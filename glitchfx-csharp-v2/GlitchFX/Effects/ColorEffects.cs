...
namespace GlitchFX.Effects
{
    public class ColorGrade : BaseEffect
    {
        ...
        public override Mat Apply(Mat frame, double time)
        {
            ...
            Cv2.Split(hsv, out var hsvCh);
            hsvCh[0].ConvertTo(hsvCh[0], MatType.CV_32FC1, 1.0, hue);
            using (var hMod = hsvCh[0])
            {
                Cv2.Merge(hsvCh, hsv);
            }
            using var backToBgr = new Mat();
            Cv2.CvtColor(hsv, backToBgr, ColorConversionCodes.HSV2BGR);
    ...
    public class Posterize : BaseEffect
    {
        ...
        public override Mat Apply(Mat frame, double time)
        {
            ...
            using var f32 = new Mat();
            frame.ConvertTo(f32, MatType.CV_32FC3, 1.0 / 255.0);
            using var scaled = f32 * (levels - 1);
            using var rounded = new Mat();
            scaled.CopyTo(rounded);
            Cv2.Round(rounded, rounded); // not native; fallback below
            using var back = rounded / (levels - 1);
            var outMat = new Mat();
            ((Mat)back).ConvertTo(outMat, MatType.CV_8UC3, 255.0);
            return outMat;
        }
    }
    ...
    public class Vignette : BaseEffect
    {
        ...
        public override Mat Apply(Mat frame, double time)
        {
            ...
            mask3.ConvertTo(maskF, MatType.CV_32FC3);
            using var weighted = f32.Mul(maskF * amount + Scalar.All(1.0 - amount));
            var outMat = new Mat();
    ...
    public class Dither : BaseEffect
    {
        ...
        public override Mat Apply(Mat frame, double time)
        {
            ...
            using var f32 = new Mat();
            frame.ConvertTo(f32, MatType.CV_32FC3, 1.0 / 255.0);
            var dithered = new Mat(h, w, MatType.CV_32FC3);
    ...
    public class ColorMap : BaseEffect
    {
        ...
        public override Mat Apply(Mat frame, double time)
        {
            ...
            for (int i = 1; i <= 12; i++)
            {
                ...
                Cv2.Absdiff(outMat, new Mat(outMat.Size(), outMat.Type(), from), diff);
